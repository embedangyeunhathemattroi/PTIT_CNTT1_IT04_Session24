interface Student {  //co the sdung type Student={}
  readonly studentId: string;
  name: string;
  email: string;
  hasCompleted: boolean;
  finalScore?: number; // chỉ có nếu học viên đã hoàn thành ?: la co hoac ko co cx dc khi truyen gia tri 
}
...
interface Course {
  ...
  title: string;
  instructor: string;
  students: Student[]; //mot mang chua hoc sinh
  isActive: boolean;
}
...
const CourseManager={
 ...
 function getCompletedStudents(course: Course): Student[] {
  ...
  return course.students.filter((item:Student)=>item.hasCompleted);
...
function calculateAverageScore(course: Course): number | null {
  ...
  if (completed.length === 0) return null;  //ko ai hoan thanh thi null
  //tinh tong diem
...
function printCourseInfo(course: Course, index: number): void {
...
function printCourseManager(manager: CourseManager): void {
