...
class Classroom {
    ...
    removeTeacher() {
        if (this.teacher) {
            ...
            console.log("⚠️ Lớp chưa có giáo viên.");
    }
    ...
    findStudentByGrade(min, max) {
        ...
        if (found.length > 0) {
            ...
            console.log("❌ Không có học sinh trong khoảng điểm này.");
    }
    stats() {
    ...
    showInfo() {
        ...
        if (this.students.length === 0)
            ...
            this.students.forEach(s => console.log(`- ID=${s.id} | ${s.name} (Điểm: ${s.grade})`));
    }
}
// ================================
...
while (true) {
    const choice = prompt(`===== MENU QUẢN LÝ LỚP HỌC NÂNG CAO =====
1. Thêm lớp mới
2. Thêm học sinh
3. Xóa học sinh
4. Gán giáo viên
