...
class Customer {
    private static nextId = 1; // ID tự động tăng
    public id: number;
    public name: string;
    public email: string;
...
class Account {
    private static nextId = 1001; // ID tài khoản bắt đầu từ 1001
    public id: number;
    public customer: Customer;
    public type: string; // "Tiết kiệm" hoặc "Thanh toán"
    public balance: number;

    constructor(customer: Customer, type: string, balance: number) {
...
class Bank {
    ...
    // 2. Tạo tài khoản
    createAccount(customerId: number, type: string, initialBalance: number) {
    ...
    deposit(accountId: number, amount: number) {
        ...
        if (!account) { alert("Không tìm thấy tài khoản"); return; }
    ...
    withdraw(accountId: number, amount: number) {
        const account = this.accounts.find(a => a.id === accountId);
        if (!account) { alert("Không tìm thấy tài khoản"); return; }
        if (account.balance < amount) { alert("Số dư không đủ!"); return; }
        account.balance -= amount;
    ...
    transfer(fromAccountId: number, toAccountId: number, amount: number) {
        ...
        if (!fromAccount || !toAccount) { alert("Không tìm thấy tài khoản"); return; }
        if (fromAccount.balance < amount) { alert("Số dư tài khoản nguồn không đủ!"); return; }
    ...
    switch (choice) {
        ...
        case "2":
            bankSystem.createAccount(
                parseInt(prompt("Nhập ID khách hàng:")!),
                prompt("Loại tài khoản (Tiết kiệm/Thanh toán):")!,
                parseFloat(prompt("Số dư ban đầu:")!)
            );
            break;
        case "3":
            bankSystem.deposit(
                parseInt(prompt("Nhập ID tài khoản:")!),
                parseFloat(prompt("Số tiền nạp:")!)
            );
            break;
        case "4":
            bankSystem.withdraw(
                parseInt(prompt("Nhập ID tài khoản:")!),
                parseFloat(prompt("Số tiền rút:")!)
            );
            break;
        case "5":
            bankSystem.transfer(
                parseInt(prompt("ID tài khoản nguồn:")!),
                parseInt(prompt("ID tài khoản đích:")!),
                parseFloat(prompt("Số tiền chuyển:")!)
            );
            break;
        case "6":
