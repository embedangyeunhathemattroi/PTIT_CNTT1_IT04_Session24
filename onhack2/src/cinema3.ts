...
class Customer {
    private static nextId = 1; // Biến static để tự tăng ID
    public id: number;
    public name: string;
    public email: string;
...
class Movie {
    ...
    constructor(title: string, genre: string, duration: number, ticketPrice: number, availableSeats: number) {
...
class TicketOrder {
    ...
    public movies: { movie: Movie, quantity: number }[];
    public totalAmount: number;

    constructor(customer: Customer, movies: { movie: Movie, quantity: number }[]) {
...
class Cinema {
    ...
    // Tạo đơn đặt vé
    createOrder(customerId: number, movieQuantities: { movieId: number, quantity: number }[]) {
        const customer = this.findById<Customer>(customerId, this.customers);
        if (!customer) { alert("Không tìm thấy khách hàng"); return; }

        let movieList: { movie: Movie, quantity: number }[] = [];
        for (let mq of movieQuantities) {
            const movie = this.findById<Movie>(mq.movieId, this.movies);
    ...
    // Cập nhật ghế
    updateMovieSeats(movieId: number, newSeats: number) {
    ...
    // Generic findById
    findById<T>(id: number, array: T[]): T | undefined {
        return (array as any).find((item: any) => item.id === id);
    }
}
    ...
    switch (choice) {
        ...
        case "2":
            cinema.addMovie(new Movie(
                prompt("Tên phim:")!,
                prompt("Thể loại:")!,
                parseInt(prompt("Thời lượng (phút):")!),
                parseFloat(prompt("Giá vé:")!),
                parseInt(prompt("Số ghế:")!)
            ));
            break;
        case "3":
        ...
        case "9":
            ...
            const foundCustomer = cinema.findById<Customer>(id, cinema.customers);
            const foundMovie = cinema.findById<Movie>(id, cinema.movies);
            if (foundCustomer) alert(foundCustomer.getDetails());
            else if (foundMovie) alert(foundMovie.getMovieInfo());
            else alert("Không tìm thấy");
            break;
        case "10":
            const movieInfo = cinema.findById<Movie>(parseInt(prompt("ID phim:")!), cinema.movies);
