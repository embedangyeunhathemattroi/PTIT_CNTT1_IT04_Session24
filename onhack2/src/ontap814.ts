// Lớp Customer
class Customer {
    private static autoId = 1; // Biến static dùng để tự động tăng ID cho mỗi khách hàng mới
    public id: number;         // Mã khách hàng
    public name: string;       // Tên khách hàng
    public email: string;      // Email khách hàng
    public shippingAddress: string; // Địa chỉ giao hàng
...
abstract class Product {
    private static autoId = 1; // Biến static để tự động tăng ID sản phẩm
    public id: number;         // Mã sản phẩm
    public name: string;       // Tên sản phẩm
    public price: number;      // Giá sản phẩm
    public stock: number;      // Số lượng tồn kho
    ...
    // Các phương thức bắt buộc lớp con phải định nghĩa
    abstract getProductInfo(): string;           // Lấy thông tin sản phẩm
    abstract getShippingCost(distance: number): number; // Tính phí ship
    abstract getCategory(): string;              // Lấy tên danh mục
}

// ElectronicsProduct - sản phẩm điện tử
class ElectronicsProduct extends Product {    public warrantyPeriod: number; // Thời gian bảo hành (tháng)
...
// Lớp Order - đơn hàng
class Order {
    private static autoOrderId = 1; // Tự động tăng ID đơn hàng
    public orderId: number;         // Mã đơn hàng
    public customer: Customer;      // Khách hàng đặt hàng
    public products: { product: Product; quantity: number }[]; // Danh sách sản phẩm + số lượng
...
class Store {
    public products: Product[] = [];   // Danh sách sản phẩm
    public customers: Customer[] = []; // Danh sách khách hàng
    public orders: Order[] = [];       // Danh sách đơn hàng
...
Chọn:`);
        ...
        if (!choice) break; // Nếu nhấn Cancel thì thoát
        ...
        switch (choice) {
            ...
            case "5": store.listAvailableProducts(); break; // Sản phẩm còn hàng
            ...
            case "8": store.countProductsByCategory(); break; // Thống kê sản phẩm
