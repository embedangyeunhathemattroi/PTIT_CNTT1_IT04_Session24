...
class Dog extends Pet {
    ...
    constructor(name: string, price: number, stock: number, breed: string, age: number) {
...
class Cat extends Pet {
    ...
    constructor(name: string, price: number, stock: number, color: string, gender: "Male" | "Female") {
        super(name, price, stock);
        this.color = color;
        this.gender = gender;
    }
...
class Order {
    ...
    public items: { pet: Pet; quantity: number }[];
    public totalAmount: number;

    constructor(customer: Customer, items: { pet: Pet; quantity: number }[]) {
...
class PetStore {
    ...
    addDog(name: string, price: number, stock: number, breed: string, age: number): void {
        this.pets.push(new Dog(name, price, stock, breed, age));
        alert("Đã thêm Dog!");
    }

    addCat(name: string, price: number, stock: number, color: string, gender: "Male" | "Female"): void {
        this.pets.push(new Cat(name, price, stock, color, gender));
        alert("Đã thêm Cat!");
    }

    createOrder(customerId: number, items: { petId: number; quantity: number }[]): void {
        const customer = this.findById<Customer>(customerId, this.customers);
        if (!customer) { alert("Không tìm thấy khách hàng!"); return; }

        const validItems: { pet: Pet; quantity: number }[] = [];
        for (let it of items) {
            const pet = this.findById<Pet>(it.petId, this.pets);
            if (!pet) { alert(`Không tìm thấy Pet ID=${it.petId}`); }
            if (!pet.sell(it.quantity)) { alert(`Pet ${pet.name} không đủ số lượng`); }
            validItems.push({ pet, quantity: it.quantity });
        }

        if (validItems.length === 0) { alert("Không có thú cưng hợp lệ!"); return; }
    ...
    countPetsByCategory(): void {
        const counts = this.pets.reduce((acc: any, p) => {
    ...
    updatePetStock(petId: number, newStock: number): void {
        const pet = this.findById<Pet>(petId, this.pets);
        if (!pet) { alert("Không tìm thấy Pet!"); return; }
        pet.stock = newStock;
        alert("Đã cập nhật tồn kho!");
    }

    getPetInfo(petId: number): string {
        const pet = this.findById<Pet>(petId, this.pets);
        return pet ? pet.getPetInfo() : "Không tìm thấy Pet";
    }

    findById<T extends Customer | Pet>(id: number, arr: T[]): T | undefined {
...
Chọn:
    ...
    switch (choice) {
        case "1":
            ...
            if (name && email) store.addCustomer(name, email);
            break;
        ...
        case "3":
            ...
            if (!custIdStr || !nStr) break;
            const n = parseInt(nStr);
            const items: { petId: number; quantity: number }[] = [];
            for (let i = 0; i < n; i++) {
                const petIdStr = prompt(`Pet ID #${i + 1}:`);
                const qtyStr = prompt("Số lượng:");
                if (!petIdStr || !qtyStr) continue;
        ...
        case "9":
            ...
            if (!petIdStr2 || !newStockStr) break;
