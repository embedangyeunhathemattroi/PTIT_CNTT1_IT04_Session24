...
class Movie {
    ...
    public duration: number; // Thời lượng (phút)
...
// =======================
class Showtime {
    public id: number;           // Mã suất chiếu
    public movie: Movie;         // Phim được chiếu
    public time: string;         // Thời gian chiếu
    public availableSeats: number; // Số ghế còn trống

    constructor(id: number, movie: Movie, time: string, availableSeats: number) {
        this.id = id;
        this.movie = movie;
        this.time = time;
...
// =======================
class Cinema {
    private movies: Movie[] = [];       // Danh sách phim
    private showtimes: Showtime[] = []; // Danh sách suất chiếu
    ...
    // Đặt vé
    bookTicket(showtimeId: number, seats: number): void {
