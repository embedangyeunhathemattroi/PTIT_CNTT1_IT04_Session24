...
class Pet {
    constructor(
        ...
        public age: number,
        public price: number,
        public status: string // "Còn hàng" | "Đã bán"
    ) {}

    displayInfo(): void {
        console.log(
            `ID: ${this.id} | Tên: ${this.name} | Loại: ${this.type} | Tuổi: ${this.age} | Giá: ${this.price} | Tình trạng: ${this.status}`
        );
    }
}
...
class PetStore {
    ...
    searchPetsByType(type: string): void {
    ...
    sortPetsByPrice(asc: boolean = true): void {
    ...
    // Bán thú cưng
    sellPet(petId: string, customerId: string): void {
    ...
    searchCustomer(keyword: string): void {
        const found = this.customers.filter(
            c => c.name.toLowerCase().includes(keyword.toLowerCase()) || c.phone.includes(keyword)
        );
        if (found.length === 0) {
